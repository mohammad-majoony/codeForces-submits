...
struct segmenttree
{
    ...
    int query(int start, int ending, int l, int r, int node, int val)
    {
        ...
        if (start >= l && ending <= r)
        {
            ...
            if (st[2 * node + 1] + lazy[2 * node + 1] >= val)
                return query(start, mid, l, r, 2 * node + 1, val);
            if (st[2 * node + 2] + lazy[2 * node + 2] >= val)
                return query(mid + 1, ending, l, r, 2 * node + 2, val);
            return LLONG_MAX;
        }

        int q1 = query(start, mid, l, r, 2 * node + 1, val);
        int q2 = query(mid + 1, ending, l, r, 2 * node + 2, val);
